...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        genome.fitness = 4.0
        net = modneat.nn.ExModFeedForwardNetwork.create(genome, config)
...
def run_experiment(config_file):
    ...
    p.add_reporter(modneat.FileOutReporter(True, './exmod_result.txt'))
    ...
    print('\nOutput:')
    net = modneat.nn.ExModFeedForwardNetwork.create(best_genome, config)
    for xi, xo in zip(xor_inputs, xor_outputs):
        output = net.activate(xi)
...
if __name__ == '__main__':
    ...
    config_path = os.path.join(local_dir, './config/exmodgenome_config.ini')
