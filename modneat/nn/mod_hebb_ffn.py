...
class ModHebbFFN(object):
    ...
    def activate(self, inputs):
        ...
        for node, modulatory, act_func, agg_func, bias, response, links in self.node_evals:
            for i, w in links:
                update_val = 0.1 * self.modulated_values[node] * self.values[node] * self.values[i]
    ...
    def create(genome, config):
        ...
        return ModHebbFFN(config.genome_config.input_keys, config.genome_config.output_keys, node_evals)
